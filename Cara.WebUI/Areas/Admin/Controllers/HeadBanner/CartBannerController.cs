...
using Cara.WebUI.Areas.Admin.ViewModels.HeadBanner.CartBanner;
...
namespace Cara.WebUI.Areas.Admin.Controllers.HeadBanner
{
	...
	public class CartBannerController : Controller
	{
		private ICartBannerRepository _repository;
		private readonly IWebHostEnvironment _env;
		public CartBannerController(ICartBannerRepository repository, IWebHostEnvironment env)
		{
		...
		public async Task<IActionResult> Create(CartBannerVM cartBannerVM)
		{
			if (!ModelState.IsValid)
			{
				return View();
			}
			if (cartBannerVM.Photo == null)
			{
				ModelState.AddModelError("Photo", "Select Photo");
				return View(cartBannerVM);
			}

			if (!cartBannerVM.Photo.CheckFileSize(500))
			{
				ModelState.AddModelError("Photo", "Image size must be less than 500 kb");
				return View(cartBannerVM);
			}

			if (!cartBannerVM.Photo.CheckFileFormat("image/"))
			{
				ModelState.AddModelError("Photo", "You must be choose image type");
				return View(cartBannerVM);
			}

			if (cartBannerVM.IsActive)
			{
			...
			try
			{
				filename = await cartBannerVM.Photo.CopyFileAsync(_env.WebRootPath, "admin", "assets", "database", "headbanners");
			}
			catch (Exception)
			{
			...
			CartBanner cartBanner = new()
			{
				Title = cartBannerVM.Title,
				Description = cartBannerVM.Description,
				Photo = filename,
				IsActive = cartBannerVM.IsActive
			};


			await _repository.CreateAsync(cartBanner);
